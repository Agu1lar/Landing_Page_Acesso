...
export default defineConfig<ChromaticConfig>({
  ...
  webServer: {
    command: process.env.CI
      ? 'pglite-server -m 100 -p 5433 --run "node node_modules/npm-run-all/bin/run-s/index.js db:migrate start"'
      : 'pglite-server -m 100 -p 5433 --run "node node_modules/npm-run-all/bin/run-s/index.js db:migrate dev:next"',
    url: baseURL,
    timeout: 60 * 1000,
